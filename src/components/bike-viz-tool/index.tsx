...
const BikeVizTool = ({ bikes }: { bikes: Bike[] }) => {
  ...
  useEffect(() => {
    ...
    if (currentModel && currentSize && canvas && context) {
      const bike = bikes.find((b) => b.model === currentModel);
      if (bike && bike.sizes) {
        const size = bike.sizes.find((s) => s.frameSize === currentSize);
        if (size) {
          // draw 2 wheels of 622mm diameter
